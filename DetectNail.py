        ...
        if success:
            ...
            for i, c in enumerate(contours):
                ...
                if  5000 >= cv2.contourArea(contours[i]) <= 7600 and radius[i] < 50:  
                    cv2.drawContours(skinMask2, contours_poly, i, (255,0,0))
                    cv2.circle(cropeedIMAGE, (int(centers[i][0]), int(centers[i][1])), int(radius[i]), (0,0,255), 2)
                    cv2.imshow('Contours', cropeedIMAGE)
                    #cv2.imshow('skin', skin)
